...
describe("createRegisterDevice", () => {
  ...
  describe("with a new hardware id", () => {
    it("should mint a code and create the device", async () => {
      ...
      // Act
      const registerDevice = createRegisterDevice({ client, generateCode: () => "8F3K2QLM" });
      const result = await registerDevice(validInput);

      // Assert
      expect(result).to.deep.equal({ code: "8F3K2QLM", created: true });
      expect(client.device.create.calledOnce).to.equal(true);
    });
    ...
    it("should retry with a new code when the code collides", async () => {
      ...
      // Act
      const registerDevice = createRegisterDevice({ client, generateCode: () => codes.shift()! });
      const result = await registerDevice(validInput);

      // Assert
      expect(result).to.deep.equal({ code: "ZZZZ2222", created: true });
  ...
  describe("with a known hardware id", () => {
    it("should return the same code and never create a new device", async () => {
      ...
      // Assert
      expect(result).to.deep.equal({ code: "8F3K2QLM", created: false });
